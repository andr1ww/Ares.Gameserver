#include <pch.h>
#include <Headers/Hooking/hde64.hpp>
#include <array>
#include <map>
...
std::map<uintptr_t, FHookData> InternalHooks;

void SetupJmp(uintptr_t Ptr, uint8_t* ShellcodeAddr)
{
    FHookData HookData;
...
FInstructionData FetchInstructionData(uintptr_t Stream)
{
    auto Length = 0;
...
{
    Patch<uint8_t>(Ptr, 0xC3);
}

void Hooking::RetTrue(uintptr_t Ptr)
{
    Patch<uint32_t>(Ptr, 0xC0FFC031);
    Patch<uint8_t>(Ptr + 4, 0xC3);
}
...
void Hooking::InternalRel32Hook(uintptr_t Ptr, void* Detour, void** Original)
{
    ...
    Patch<uint32_t>(Ptr, (uint32_t)NewRel32);
}