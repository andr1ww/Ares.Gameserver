...
namespace UC
{
    ...
    {
    public:
        static inline auto InternalRealloc = (void* (*)(void*, __int64, unsigned int))(ImageBase + 0x21CF6F0);
    ...
    inline constexpr void DestructItems(ElementType* Element, SizeType Count)
    {
    }
    template <typename ElementType, typename SizeType>
        requires(sizeof(ElementType) > 0 && !std::is_trivially_destructible_v<ElementType>) // the sizeof here should improve the error messages we get when we try to call this function with incomplete types
    ...
    public:
        ...
        using iterator = Iterators::TArrayIterator<ArrayElementType>;
        using const_iterator = Iterators::TArrayIterator<const ArrayElementType>;
    ...
    public:
        ...
        TArray(const TArray& Other)
        {
            ...
            if (MaxElements)
            {
                Data = FMemory::MallocForType<ArrayElementType>(MaxElements);

                memcpy(Data, Other.Data, Other.NumElements * sizeof(ArrayElementType));
            }
            else
                Data = nullptr;
        }
    ...
    public:
        ...
        inline ArrayElementType& Add(ArrayElementType&& Element)
        {
            Reserve(1);

            new (GetData() + NumElements) ArrayElementType(std::move(Element));
            NumElements++;
        ...
        inline ArrayElementType& Add(const ArrayElementType& Element)
        {
            ...
            new (GetData() + NumElements) ArrayElementType(Element);
            NumElements++;
            // return Data[NumElements];
            //_TArrayAdd((void*&)Data, NumElements, MaxElements, sizeof(ArrayElementType), &Element);
            return Data[NumElements - 1];
...
#pragma warning(suppress : 4244)
        ...
        inline std::string ToStr() const /* hopefully this shit ffucking wokrsksrkrskrskrsksrk!! */
        {
            if (*this)
            {
                std::wstring WData(Data);
