...
namespace Offsets
{
	constexpr int32 GObjects          = 0x06CA3C00;
	constexpr int32 AppendString      = 0x0227F6D0;
	constexpr int32 GNames            = 0x06C6AA80;
	constexpr int32 GWorld            = 0x00000000;
	constexpr int32 ProcessEvent      = 0x02478CA0;
...
public:
    ...
    int32 SerialNumber;
	uint8                                         Pad_8[0x104];                                      // 0x0008(0x0110)(Fixing Struct Size After Last Property [ Dumper-7 ])
};
static_assert(alignof(FUObjectItem) == 0x000008, "Wrong alignment on FUObjectItem");
static_assert(sizeof(FUObjectItem) == 0x000118, "Wrong size on FUObjectItem");
...
private:
	static inline uint64_t DecryptPointer(__int64 a1)
	{
		...
		uint64_t hashed = seed64 ^ (seed64 >> 15);
		hashed = seed64 ^ (hashed >> 12) ^ ((uint64_t)(seed << 25));
		uint64_t v5 = 0x2545F4914F6CDD1DULL * hashed;

		uint32_t v7 = (uint32_t)(v5 >> 32);
		uint64_t div7 = v5 / 7; // sarah: was ((__uint128_t)v5 * 0x2492492492492493ULL) >> 64
		uint32_t hashed32 = (uint32_t)hashed;
		uint64_t v8 = (uint64_t)(int64_t)(1332534557u * hashed32 - 7u * (uint32_t)((div7 + ((v5 - div7) >> 1)) >> 2));

		uint64_t v9 = *(uint64_t*)(a1 + 8 * v8 + 64);
		uint64_t v10 = v9;
		uint32_t v11 = (uint32_t)v8 % 7;
		uint32_t v12 = v7 + (uint32_t)v8;
		...
		if (v11 == 3)
		{
			uint64_t t = ((v10 << 1) & 0xAAAAAAAAAAAAAAAAULL) | ((v10 >> 1) & 0x5555555555555555ULL);
			v10 = std::rotr(t, (uint8_t)((uint32_t)v12 % 0x3F) + 1);
		}
		else if (v11 == 4)
		{
			uint64_t t = ((v10 << 1) & 0xAAAAAAAAAAAAAAAAULL) | ((v10 >> 1) & 0x5555555555555555ULL);
		...
		if (v11 == 5)
		{
			v10 = ~v10 - (uint64_t)v12;
		}
		else if (v11 == 6)
		{
			uint64_t t = v9;
			t = ((t << 1) & 0xAAAAAAAAAAAAAAAAULL) | ((t >> 1) & 0x5555555555555555ULL);
			t = ((t << 2) & 0xCCCCCCCCCCCCCCCCULL) | ((t >> 2) & 0x3333333333333333ULL);
			t = ((t << 4) & 0xF0F0F0F0F0F0F0F0ULL) | ((t >> 4) & 0x0F0F0F0F0F0F0F0FULL);
			uint64_t swapped = ((t << 8) & 0xFF00FF00FF00FF00ULL) | ((t >> 8) & 0x00FF00FF00FF00FFULL);
			v10 = v12 + std::rotl(swapped, 32);
		}

		return v10 ^ (uint64_t)seed;
	}

	inline void InitGObjects()
	{
		GObjectsAddress = reinterpret_cast<void*>(DecryptPointer(InSDKUtils::GetImageBase() + Offsets::GObjects));
	}
...
public:
    ...
    {
        static auto FName__Ctor = (void (*)(FName*, const wchar_t*, int))(ImageBase + 0x227BCC0);
    ...
    FName(int32 InComparisonIndex = 0, int32 InDisplayIndex = 0, int32 InNumber = 0)
...
public:
    ...
    {
        static auto Ctor = (UObject * (*)(const FWeakObjectPtr*, UObject*))(ImageBase + 0x24926E0);
...
public:
    UObject* Get() const
    {
        ...
        auto LoadSynchronous = (UObject * (*)(const FSoftObjectPtr*))(ImageBase + 0x15A16D0);
